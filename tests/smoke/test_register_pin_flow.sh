...
AUTH_FOR_PHONE=$(spacetime sql "$DB" "SELECT wallet_address FROM esim_profile WHERE phone_number = '$PHONE_NORM'" | grep -oE '"[0-9a-f]{40}"' | tr -d '"' || true)
if [ -n "$AUTH_FOR_PHONE" ]; then
    spacetime sql "$DB" "DELETE FROM auth_7702 WHERE authority_address = '$AUTH_FOR_PHONE'" || true
fi
...
PROFILE_COUNT=$(spacetime sql "$DB" "SELECT phone_number FROM esim_profile WHERE phone_number = '$PHONE_NORM'" | grep -cE "^[[:space:]]*\"$PHONE_NORM\"" || true)
[ "$PROFILE_COUNT" = "1" ] || { echo "FAIL: expected 1 esim_profile row, got $PROFILE_COUNT"; exit 1; }

PIN_COUNT=$(spacetime sql "$DB" "SELECT phone_number FROM user_pin WHERE phone_number = '$PHONE_NORM'" | grep -cE "^[[:space:]]*\"$PHONE_NORM\"" || true)
[ "$PIN_COUNT" = "1" ] || { echo "FAIL: expected 1 user_pin row, got $PIN_COUNT"; exit 1; }

AUTH_COUNT=$(spacetime sql "$DB" "SELECT authority_address FROM auth_7702" | grep -cE '^[[:space:]]*"[0-9a-f]{40}"' || true)
