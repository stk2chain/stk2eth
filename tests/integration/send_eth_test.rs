...
use std::str::FromStr;
use ussdgeth::{send_eth, Swap, USSDSession};
...
mod send_eth_tests {
    ...
    fn test_send_eth_reducer_fails_initially() {
        ...
        assert!(
            result.is_err(),
            "send_eth reducer should fail - not implemented yet"
        );
    }
    ...
    fn test_send_eth_validates_addresses() {
        ...
        let result = std::panic::catch_unwind(|| {
            send_eth(
                ...
                "0x8ba1f109551bD432803012645Hac136c6b3d283c".to_string(),
                "1.0".to_string(),
            )
        });
