...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "\n Step 2: Executing {} transfers via send_eth reducer",
        transfers_to_test
    );
    let transfer_results =
        execute_transfers(spacetimedb_url, &test_accounts, transfers_to_test).await?;
    ...
    let validation_result =
        validate_balance_deltas(&initial_balances, &final_balances, &transfer_results);
